...
- **Configurable Access Control** — Generate shareable file URLs with admin-controlled access for authenticated and unauthenticated users
- **Drag-and-Drop Upload** — Interactive upload area with real-time progress bar
- **Role-Based Access Control** — Admins manage files and control access settings; authenticated users always view files; unauthenticated user access is admin-configurable
...
| Path Prefix | Optional folder prefix for uploaded files (e.g. `uploads/`) || Allow Unauthenticated Access | Enable public access to shared file URLs (disabled by default) |
...
Share this URL with others. When they visit the URL, the following applies:
- **Logged-in users** are always redirected to a time-limited presigned S3 URL (5-minute expiry)
- **Logged-out users**: Behavior depends on the "Allow Unauthenticated Access" setting:
  - If disabled (default): redirected to the WordPress login page
  - If enabled: granted direct access to a time-limited presigned S3 URL (5-minute expiry)
...
| Unauthenticated | No | No | If enabled by admin |
